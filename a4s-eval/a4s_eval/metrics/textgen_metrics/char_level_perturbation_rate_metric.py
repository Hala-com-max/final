...
class A4SClassificationWrapper(ModelWrapper):
    ...
    def _create_prompt(self, text: str) -> str:
        ...
        return (
            "SYSTEM INSTRUCTION: Classify the text into one of the following labels: "
            f"{allowed_labels}.\n"
    ...
    def __call__(self, text_list: list[str]) -> np.ndarray:
        ...
        for text in text_list:
            ...
            try:
                ...
                predicted_label = raw_prediction.strip().upper()
...
def _compute_attack_metrics(
    ...
    for result in results:
        ...
        perturbation_rate = (
            levenshtein_distance(original_text, perturbed_text) / original_length
        )

        perturbation_rates.append(perturbation_rate)

    pd.DataFrame(attack_examples).to_csv(
        config.attack_examples_path,
        index=False,
    )
...
def strong_deepwordbug_metric(
    ...
    """
    Runs a TextAttack DeepWordBug-style character-level attack and reports the
    average character perturbation rate over successful attacks.
